...
async function startServer() {
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      const systemInstruction = 
        "You are the 'Visit Sri Lanka Guide AI Assistant,' an expert, friendly local travel advisor. " +
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Visit Sri Lanka Guide Server listening dynamically on http://0.0.0.0:${PORT}`);
  });
}
